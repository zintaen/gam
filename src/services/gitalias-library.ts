/**
 * GitAlias Library Service
 *
 * Provides all predefined aliases from https://github.com/GitAlias/gitalias
 * Data is statically embedded — no runtime fetch needed.
 */
...
const LIBRARY: I_LibraryAlias[] = RAW.map(([name, command, category, description]) => ({
    name,
    command,
    category,
    description,
...
export function getAllAliases(): I_LibraryAlias[] {
    return LIBRARY;
}
...
export function getByCategory(category: string): I_LibraryAlias[] {
    return LIBRARY.filter(a => a.category === category);
}

/** Search aliases by query (matches name, command, category, or description). */
export function searchLibrary(query: string): I_LibraryAlias[] {
